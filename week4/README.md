...
- `APL.Eval`: To make testing easier, you should replace the definition of
  ...
  cool to see that you don't have to change your version of `eval` from A2 at
  all despite the fact that the underlying monad will change rather
  significantly.
...
In this task, you'll develop a free monad-based version of the `EvalM`
evaluation monad. The first thing to do is define an effect type for`EvalM`,
which will include a constructor for each kind of effect we want the `EvalM`
